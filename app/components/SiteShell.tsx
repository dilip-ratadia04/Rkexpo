  ...
  return (
    <>
      <div className="trade-strip">
        <span>Indian origin · Global standards</span>
        <span className="trade-strip-right">APEDA-ready documentation · FOB / CIF</span>
      </div>
  ...
  return (
    <footer className="site-footer">
      ...
      <div className="footer-grid page-shell">
        <div>
          <Brand />
          <p className="footer-copy">Traceable Indian grains, prepared for modern international supply chains.</p>
        </div>
        <div>
          <p className="footer-label">Explore</p>
          {navItems.slice(1).map((item) => <Link href={item.href} key={item.href}>{item.label}</Link>)}
        </div>
        <div>
        ...
        <div>
          <p className="footer-label">Trade desk</p>
          <span>Mon–Sat · 09:00–18:00 IST</span>
          <span>FOB Mundra / Nhava Sheva</span>
        </div>
      </div>
...
export function ProductVisual({ className, label }: { className: string; label: string }) {
  return (
    <div className={`product-visual ${className}`} role="img" aria-label={`${label} grain illustration`}>
